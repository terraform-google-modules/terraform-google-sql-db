...
| backup\_configuration | The backup\_configuration settings subblock for the database setings | <pre>object({<br>    binary_log_enabled             = bool<br>    enabled                        = bool<br>    start_time                     = string<br>    location                       = string<br>    transaction_log_retention_days = string<br>    backup_retention_settings      = map(string)<br>  })</pre> | <pre>{<br>  "backup_retention_settings": null,<br>  "binary_log_enabled": false,<br>  "enabled": false,<br>  "location": null,<br>  "start_time": null,<br>  "transaction_log_retention_days": null<br>}</pre> | no |
