...
locals {
  ...
  iam_users = { for iu in var.iam_users : iu.name => iu }
}
...
resource "google_sql_user" "iam_account" {
  ...
  name = each.value.is_account_sa ? (
    trimsuffix(google_service_account.sa[each.value.name].email, ".gserviceaccount.com")
    ) : (
    each.value.name
  )
  instance = google_sql_database_instance.default.name
...
resource "google_service_account" "sa" {
  for_each = {
    for i in local.iam_users : i.name => i
    if i.is_account_sa
  }
...
resource "google_project_iam_member" "iam_binding" {
  ...
  member = each.value.is_account_sa ? (
    "serviceAccount:${google_service_account.sa[each.value.name].email}"
    ) : (
