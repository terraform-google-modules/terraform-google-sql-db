...
func TestPostgreSqlHaModule(t *testing.T) {
	...
	pSql.DefineVerify(func(assert *assert.Assertions) {
		...
		for _, instance := range instaceNames {
			...
			assert.Equal(authNetworkMap["name"].String(), fmt.Sprintf("%s-cidr", pSql.GetStringOutput("project_id")), "Expected auth network")
			assert.Equal(authNetworkMap["value"].String(), pSql.GetStringOutput("authorized_network"), "Expected auth network within cdir range")
			...
			if isMaster {
				...
				assert.Equal(len(op.Array()), 4, "Expected at least 4 users")
			}
		}
	})
