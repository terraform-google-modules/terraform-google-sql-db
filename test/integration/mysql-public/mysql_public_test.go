...
import (
	// import standard formatting and testing packages
	"fmt"
	"testing"
...
func TestMySqlPublicModule(t *testing.T) { // name the function as Test*
	...
	mySqlT.DefineVerify(func(assert *assert.Assertions) { // define and write a custom verifier for this test case
		...
		// use the op object (a gjson struct) to parse through the JSON results and assert the values of the resource against the constants defined above
		assert.Equal(databaseVersion, op.Get("databaseVersion").String(), "database versions is valid is set to "+databaseVersion)
		assert.Equal(gceZone, op.Get("gceZone").String(), "GCE zone is valid")
		...
		assert.Equal(true, op.Get("settings.ipConfiguration.ipv4Enabled").Bool(), "ipv4 is enabled")
